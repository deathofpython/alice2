...
from params import get_map_params, get_geocoder_params
...
def geocoder_answer(coords):
    response = requests.get(geocoder_api_server, params=get_geocoder_params(coords)).json()
...
def main():
    ...
    while pygame.event.wait().type != pygame.QUIT:
        try:
            ...
            ex.label.setText(geocoder_answer(','.join([str(coords[0]), str(coords[1])])))
        except:
            print('wrong coords')
